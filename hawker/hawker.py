...
# Checks that dateTime is infact a dataTime and not a string:
# Gets called in dateTime sorted() in hawk()
def to_dt(val):
...
def hawk(num_of_articles): #Prints the dictonary meal, containing all feeds
    ...
    sorted_articles = sorted(
            all_articles,
            key=lambda x: to_dt(x.get("Published")), #to_dt() checks and returns dateTime
            reverse=False
            )
